...
{
	// the aplication can use this function for debuging purpose by writing the 
	// face to a global variable then display which face of the mesh are been used for collition.
    // ,,,,,,,,,,,
	// ..........

	// the application can use this function also to modify the collsion geometry by changing the face ID
	// this could be usesfull to make surface change for example from dry to wet, or breaking glass
	// for this the application should used the functions:
	// int id = NewtonTreeCollisionGetFaceAtribute (treeCollision, indexArray); 
	// NewtonTreeCollisionSetFaceAtribute (treeCollision, indexArray, newid);
...
void InitScene()
{
	...
	level = new LevelPrimitive ("level.dat", nWorld, LevelCollisionCallback);
//	level = new LevelPrimitive ("xxx.dat", nWorld, LevelCollisionCallback);
