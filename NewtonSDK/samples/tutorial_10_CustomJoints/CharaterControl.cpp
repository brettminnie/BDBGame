...
//////////////////////////////////////////////////////////////////////
CharacterController::CharacterController (NewtonWorld* nWorld, const dVector& position, const dVector& size)
	:SpherePrimitive (GetIdentityMatrix(), size.m_x * 0.5f, size.m_y * 0.5f, size.m_z * 0.5f), 
...
{							    
	...
	// asume the chaterte is on the air
	m_isAirBorne = true;
	m_isStopped = true;
	m_jumpTimer = 0;
	...
	// calculate teh character ellise radius
	dVector radius (size.Scale (0.5f));

	// svale the Max translatation vefore considering tunneling prevention 
	...
	// wrap the contater collision under a transform, modifier for tunneling trught walls avoidance
	...
	damp[1] = 0.0f;
	damp[2] = 0.0f;
	// set the voicous daming the the minimum
...
dFloat CharacterController::FindFloor (NewtonWorld* world, const dVector& p0, dFloat maxDist)
{
	...
	NewtonWorldRayCast (world, &p0[0], &p1[0], OnRayCastPlacement, &data);
...
// this function will analyse the contacts and determine waht action is to be taken
...
// //////////////////////////////////////////////////////////////////////////////////////// 
// 
// this is the main charater controller function.
// This is the strategy
// - the charter momenet is contsraint to an up vetor joint
// - horizontal linear motion will be controlled by forces.
// - vertical momevent is control kenematically when the body is no in free fall
// - turning is controlled kinematically al all time
//
// - To preven tunneling trught walls the collision geometry will be scaled by the translation distance when 
//   the charatent is moving a hioght speed
void CharacterController::OnApplyForceAndTorque ()
{
	...
	// apply the gravity force, cheat a litle with the chareter grabity
	force = dVector (0.0f, -mass * 20.0f, 0.0f);

	// Get the velocity vector
	NewtonBodyGetVelocity(m_myBody, &velocity.m_x);

	// determine if the character have to be snacp to the forund
	NewtonBodyGetMatrix(m_myBody, &matrix[0][0]);


	// if the floor is with in reach then the character muxt be snapto the ground
	// the max allow distance for snaping i 0.25 of a meter
	if (m_isAirBorne && !m_jumpTimer) { 
		...
		if ((deltaHeight < (0.25f - 0.001f)) && (deltaHeight > 0.01f)) {
			// snap to floor ony if the floor is lower than the charcter feets		
	...
	heading = heading.Scale (1.0f / sqrtf ((heading % heading) + 1.0e-6f));
	...
	if (dist > m_maxTranslation * m_maxTranslation) {
		// when the velocity is high enoght that can miss collision we will enlarge the collision 
		// long the vectior velocity
		dist = sqrtf (dist);
		...
		// get the transprose of the matrix
		dMatrix transp (collisionPaddingMatrix.Transpose());
		transp[0] = transp[0].Scale (dist/m_maxTranslation);

		// calculate a oblique svale matrix by using a similar tranfomatrip matrix of the for, R'* S * R
...
//	torque = dVector (0.0f, Iyy * (20.0f * steerAngle - 5.0f * omega.m_y), 0.0f);  
	...
	// asume the chaterter is on the air. this variable will be set to false if the contact detect 
	...
	if (animateUpVector) {
		// animater the pin vector
		m_yawAngle = fmodf (m_yawAngle + 3.0f * 3.1416f / 180.0f, 2.0f * 3.1416f);
		dMatrix animMatrix (dgYawMatrix(m_yawAngle));
		dVector dir (sinf (20.0f * 3.1416f / 180.0f), cosf (20.0f * 3.1416f / 180.0f), 0.0f);
