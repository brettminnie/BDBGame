...
static debugRayCast = 0;
dVector debugRayCastLines[1024][2];


HeightFieldCollision::HeightFieldCollision(NewtonWorld* nWorld, NewtonBodyDestructor destructor)
...
//	m_lastFaceCount = 0;
	...
	for (z = 0; z < HEIGHT_SIZE; z ++) {
		for (x = 0; x < HEIGHT_SIZE; x ++) {
			// add some pseudo gaussing randoness to the height field
			dFloat r = dFloat (dRand() + dRand()) / dRAND_MAX - 1.0f;
			m_heightField[z][x] = r * 0.125f;

			// add sine wave for rolling terrain
			m_heightField[z][x] += (5.0f * sinf(x/5.0f) + 7.0f * cosf((z)/7.0f)) * 0.5f;
		}
	}
...
void HeightFieldCollision::CalculateMinExtend2d (const dVector& p0, const dVector& p1, dVector& boxP0, dVector& boxP1)
{
	...
	x0 = CELL_SIZE * floorf (x0 * (1.0f / CELL_SIZE));
	z0 = CELL_SIZE * floorf (z0 * (1.0f / CELL_SIZE));
	x1 = CELL_SIZE * floorf (x1 * (1.0f / CELL_SIZE)) + CELL_SIZE;
	z1 = CELL_SIZE * floorf (z1 * (1.0f / CELL_SIZE)) + CELL_SIZE;
...
void HeightFieldCollision::CalculateMinExtend3d (const dVector& p0, const dVector& p1, dVector& boxP0, dVector& boxP1)
{
	...
	x0 = CELL_SIZE * floorf (x0 * (1.0f / CELL_SIZE));
	y0 = CELL_SIZE * floorf (z0 * (1.0f / CELL_SIZE));
	z0 = CELL_SIZE * floorf (z0 * (1.0f / CELL_SIZE));

	x1 = CELL_SIZE * floorf (x1 * (1.0f / CELL_SIZE)) + CELL_SIZE;
	y1 = CELL_SIZE * floorf (y1 * (1.0f / CELL_SIZE)) + CELL_SIZE;
	z1 = CELL_SIZE * floorf (z1 * (1.0f / CELL_SIZE)) + CELL_SIZE;
...
// calculate the interstion point of a line segment and the two triangles making a the cell of a heih pam terrain
dFloat HeightFieldCollision::RayCastCell (int xIndex0, int zIndex0, const dVector& p0, const dVector& dp, dVector& normalOut)
{
	dFloat t;
...
// determine if a ray segment interset the height map cell
...
//if (q1.m_y < 0.0f) return q0.m_y / (q0.m_y - q1.m_y);
	...
	dVector dq (q1 - q0);
	dVector padding (dq.Scale (CELL_SIZE * 10.0f / (sqrtf (dq % dq) + 1.0e-6f)));
	...
	if (map->ClipRay2d (p0, p1, boxP0, boxP1)) {
		...
		ix0 = floorf (p0.m_x * invScale);
		iz0 = floorf (p0.m_z * invScale);
		...
		do {
			...
			if (t < 1.0f) {
				...
				normalOut = normalOut.Scale (1.0f / sqrtf (normalOut % normalOut));
...
void  HeightFieldCollision::MeshCollisionCollideCallback (NewtonUserMeshCollisionCollideDesc* collideDesc)
{
	...
	x0 = (int) (boxP0.m_x * (1.0f / CELL_SIZE));
	x1 = (int) (boxP1.m_x * (1.0f / CELL_SIZE));

	z0 = (int) (boxP0.m_z * (1.0f / CELL_SIZE));
	z1 = (int) (boxP1.m_z * (1.0f / CELL_SIZE));
	...
	// scan the vertices entersected by the box extend
	vertexIndex = 0;
	for (z = z0; z <= z1; z ++) {
		for (x = x0; x <= x1; x ++) {
...
//		int id = (int)NewtonConvexCollisionGetUserID (NewtonBodyGetCollision (collideDesc->m_objBody));
//		if (id  == 0x100) {
			if (hightMap_debugCount < 1024 * 4 - 256) {
				...
				for (int i = 0; i < faceCount; i ++) {
					...
					for (int j = 0; j <  map->m_faceIndices[i]; j ++) {
