	...
	:NewtonCustomJoint(5, child, parent)
{
	...
	// use as primary pin vector the line that goes forn the pivot to the oribgin of body zero
	// this eliminates some offeset unwanted torques 
	NewtonBodyGetMatrix(m_body0, &matrix[0][0]);

	dVector pin (matrix.m_posit - pivot); 
	if ((pin % pin) < 1.0e-3f) {
		// the the pivot is already at the oringin then any pin will do
		pin = matrix.m_front;
	}
...
void CustomBallAndSocket::SubmitConstrainst ()
{
	...
	// Restrict the movemnet on the pivot point along all tree orthonormal direction
